...
Deno.serve(async (req: Request) => {
  ...
  if (!kakaoUser) {
    return json({ error: "invalid_kakao_access_token" }, 401);
  }

  if (!kakaoUser.id) {
    return json({ error: "invalid_kakao_user" }, 401);
  }
  ...
  const { error: userError } = await admin.from("app_users").upsert(
    {
      id: userId,
      provider: "kakao",
  ...
  if (userError) {
    return json({ error: "user_upsert_failed", detail: userError.message }, 500);
  }
  ...
  if (sessionError) {
    return json({ error: "session_upsert_failed", detail: sessionError.message }, 500);
  }
...
async function fetchKakaoUser(accessToken: string): Promise<KakaoUser> {
  ...
  if (!response.ok) {
    throw new Error(`kakao_user_fetch_failed:${response.status}`);
  }
